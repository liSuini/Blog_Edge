...
export async function listKeys(prefix) {
  ...
  // EdgeOne pages-blob list() 返回 { blobs: [{key, etag}], directories: [...] }
  if (result?.blobs) return result.blobs.map((b) => b.key);
  // 兼容旧格式 { keys: [...] }
  if (result?.keys) return result.keys.map((k) => (typeof k === "string" ? k : k.name));
  // 兼容纯数组格式
  if (Array.isArray(result)) return result.map((k) => (typeof k === "string" ? k : k.key || k.name));
  return [];
}
